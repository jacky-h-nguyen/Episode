...
class Router {
    ...
    constructor(app, db) {
        ...
        this.search(app, db);
        this.insertShow(app, db);
        this.deleteShow(app, db);
    }
        ...
        app.post("/doInsertShow", (req, res) => {
            ...
            db.query("SELECT arr FROM user WHERE username = ? LIMIT 1", cols, (err, result, fields) => {
                if (result[0].arr != "null") {
                    ...
                    let sql = 'UPDATE user SET arr = ? WHERE username = ?';
                    var y = [{ "show": req.body.item1, "imageURL": req.body.item2}];
    ...
    search(app, db) {
        app.post("/search", (req, res) => {
            ...
            db.query("SELECT * FROM showslist WHERE LOWER( showslist.show ) LIKE '%" + term + "%'", function (err, result) {
